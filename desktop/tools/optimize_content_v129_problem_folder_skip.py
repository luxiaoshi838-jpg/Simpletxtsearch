...
# Inject live status detail after the configured-exclusion count if present.
status_anchor = '                if (directorySnapshot > 0) status.append("，排除目录 ").append(directorySnapshot).append(" 个");\n'
if status_anchor in text:
    text = text.replace(
        status_anchor,
        status_anchor +
        '                if (problemFolderSnapshot > 0) status.append("，问题文件夹自动跳过 ").append(problemFolderSnapshot).append(" 个");\n',
        1
    )
else:
    # v1.2.8 compact status is a setText expression; append a second path line instead.
    pass

# The second status line should prioritize the most recent problem folder if one exists.
replace_once(
    '                currentPathLabel.setText("当前：" + abbreviatePath(folderSnapshot) + " ｜ 最近文件：" + abbreviatePath(pathSnapshot));\n',
    '                currentPathLabel.setText("当前：" + abbreviatePath(folderSnapshot) + " ｜ 最近文件：" + abbreviatePath(pathSnapshot)\n'
